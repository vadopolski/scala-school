...
object ChatServer {
  ...
  def chatUser(hub: ActorRef[Hub.Message]): Flow[Message, Message, NotUsed] =
    Flow
      .lazyInitAsync(() =>
        (hub ? Hub.NewSession).map { sessionRef =>
          ...
          val output: Source[Message, NotUsed] =
            Source
              .actorRef[Out](1000, OverflowStrategy.dropHead)
  ...
  def main(args: Array[String]): Unit = {
    Http()
      .bindAndHandle(route, "0.0.0.0", 9889)
      .flatMap { bnd =>
